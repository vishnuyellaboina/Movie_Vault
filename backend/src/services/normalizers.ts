...
function getProfileUrl(path?: string | null, size = "w342") {
  return path ? `https://image.tmdb.org/t/p/${size}${path}` : undefined;
}
...
function normalizeBoxOffice(movie: any, omdb?: any) {
  ...
  if (typeof movie.revenue === "number" && movie.revenue > 0) {
    return formatUsdAmount(movie.revenue);
  }
...
export function normalizeMovieDetail(movie: any, credits: any, providers: any, omdb?: any): MovieDetail {
  return {
...
export function buildMoviePage(payload: any): MoviePage {
  return {
    page: payload.page,
...
export function buildHomeFeed(feed: HomeFeed): HomeFeed {
  return feed;
}
